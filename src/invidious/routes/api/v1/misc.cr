module Invidious::Routes::API::V1::Misc
  ...
  def self.get_playlist(env : HTTP::Server::Context)
    ...
    if offset > 0
      ...
      response = playlist.to_json(offset - lookback, shuffle_videos: shuffle_videos)
      ...
      lookback = 0
      response = playlist.to_json(offset, video_id: video_id, shuffle_videos: shuffle_videos)
      json_response = JSON.parse(response)
